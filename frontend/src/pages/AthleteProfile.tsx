...
const AthleteProfile: React.FC = () => {
  ...
  if (!athlete) return <div className="p-4 text-center">Athlete not found.</div>;

  return (
    <div className="p-4">
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-3xl font-bold">{athlete.name}'s Profile</h1>
        ...
        <div className="bg-white p-6 rounded-lg shadow-md md:col-span-2">
          ...
          {athlete.training_sessions?.length > 0 ? (
            <ul className="space-y-4">
              {athlete.training_sessions.map((session: any) => (
      ...
      <div className="bg-white p-6 rounded-lg shadow-md">
        ...
        {athlete.performance_metrics?.length > 0 ? (
          <ul className="space-y-4">
            {athlete.performance_metrics.map((metric: any) => (
              <li key={metric.id} className="p-4 bg-gray-50 rounded-lg border">
                <p className="font-semibold">
                  {metric.metric_name}: {metric.metric_value}
                </p>
                {metric.notes && <p className="text-sm text-gray-600 italic mt-1">Notes: {metric.notes}</p>}
