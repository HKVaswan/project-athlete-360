-- 0002_seed_data_updated.sql
-- Seed data for Athlete Management System (demo)
-- Assumes schema from 0001_init_schema.sql has been applied.
...
-- Seed user roles
-- -----------------------
INSERT INTO user_role (id, code, name)
VALUES
  ('939a9c40-3b4a-4b7b-8b5e-14300a6e8f42', 'admin', 'Administrator'),
  ('d88f619a-9e5c-4b5b-8f3a-69486c4a3d4f', 'coach', 'Coach'),
  ('e3a9c7b0-1f9e-4b4e-98b4-83955d5b12a8', 'physio', 'Physiotherapist'),
  ('f7c3b9d1-8b2c-4b3c-9b8a-11270e5f2a1d', 'parent', 'Parent/Guardian'),
  ('a5b7d9f2-0c1d-4b8a-9c7d-30485f6a9c8d', 'athlete', 'Athlete')
ON CONFLICT (code) DO NOTHING;

-- -----------------------
-- Seed institutions
-- -----------------------
INSERT INTO institution (id, name, address, timezone)
VALUES
  ('11111111-1111-1111-1111-111111111111', 'Starlight High School', '123 Main St, City', 'Asia/Kolkata'),
  ('22222222-2222-2222-2222-222222222222', 'Riverside Academy', '45 River Rd, City', 'Asia/Kolkata')
ON CONFLICT (id) DO NOTHING;

-- -----------------------
-- Seed sports
-- -----------------------
INSERT INTO sport (id, institution_id, code, name)
VALUES
  ('aaaa0000-0000-4000-8000-000000000001', '11111111-1111-1111-1111-111111111111', 'football', 'Football'),
  ('aaaa0000-0000-4000-8000-000000000002', '11111111-1111-1111-1111-111111111111', 'athletics', 'Athletics'),
  ('aaaa0000-0000-4000-8000-000000000003', '22222222-2222-2222-2222-222222222222', 'basketball', 'Basketball')
ON CONFLICT (code) DO NOTHING;

-- -----------------------
-- Seed app users
-- -----------------------
INSERT INTO app_user (id, institution_id, email, full_name, role_id, phone, password_hash)
VALUES
...
-- Seed athletes
-- -----------------------
INSERT INTO athlete (id, institution_id, user_id, first_name, last_name, dob, gender, primary_sport_id, grade, emergency_contact)
VALUES
...
-- Seed teams
-- -----------------------
INSERT INTO team (id, institution_id, name, sport_id, season)
VALUES
...
-- Seed roster
-- -----------------------
INSERT INTO roster (id, team_id, athlete_id, jersey_no, role, joined_at)
VALUES
...
-- Seed sessions
-- -----------------------
INSERT INTO session (id, team_id, coach_id, title, start_ts, end_ts, location, notes)
VALUES
...
-- Seed attendance
-- -----------------------
INSERT INTO attendance (id, session_id, athlete_id, status, recorded_by, recorded_at)
VALUES
  ...
  (gen_random_uuid(), (SELECT id FROM session WHERE team_id='40000000-0000-0000-0000-000000000001' LIMIT 1), '30000000-0000-0000-0000-000000000002', 'present', '100000